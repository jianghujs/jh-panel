...
class cert_api:
    ...
    def __init__(self):
        self.__user_agent = 'JH-Panel:' + mw.getRandomString(8)
...
'''
    ...
    def renewCertTo(self, domains, auth_type, auth_to, index=None):
        ...
        try:
            ...
            mw.notifyMessage(msg=notify_msg, title="续签成功：{} | {}".format(domains[0], mw.getConfig('title')), stype='续签完成', trigger_time=0)
        except Exception as e:
            if str(e).find('429') > -1:
                msg = '至少超过7天，才能续期!'
                writeLog("|-" + msg)
            ...
            mw.notifyMessage(msg=notify_msg, title="续签失败：{} | {}".format(domains[0], mw.getConfig('title')), stype='续签失败', trigger_time=0)
