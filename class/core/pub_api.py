...
class pub_api:
    ...
    def doSiteLoginApi(self):
        ...
        login_cache_count = 5
        login_cache_limit = session.get('site_login_cache_limit_' + site_domain, 0)
        ...
        if not user_match:
            ...
            session['site_login_cache_limit_' + site_domain] = login_cache_limit
            return mw.returnJson(False, mw.getInfo("用户名或密码错误,您还可以尝试[{1}]次!", (str(login_cache_count - login_cache_limit))))

        session['site_login_cache_limit_' + site_domain] = None
        session['site_login_' + site_domain] = True
        session['site_username_' + site_domain] = username
        session['site_id_' + site_domain] = site_domain
        session['site_overdue_' + site_domain] = int(time.time()) + 7 * 24 * 60 * 60

        return mw.returnJson(True, '登录成功,正在跳转...')

    def checkSiteLoginApi(self):
        site_domain = request.host
        site_login_key = 'site_login_' + site_domain
