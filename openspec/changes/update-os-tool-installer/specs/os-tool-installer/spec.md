...
系统 SHALL 提供 `scripts/os_tool/install.sh` 安装脚本，将 `scripts/os_tool` 下的全部脚本安装到 `/www/server/os_tool`，且不覆盖该目录中的其他无关文件。

#### Scenario: 安装脚本
- **WHEN** 用户运行 `bash scripts/os_tool/install.sh`
- **THEN** `scripts/os_tool` 下的脚本被复制到 `/www/server/os_tool`
- **AND** `/www/server/os_tool` 下与 os_tool 无关的文件保持不变

### Requirement: 提供 jht 命令入口
系统 SHALL 安装 `jht` 命令并启动 `scripts/os_tool/index.sh` 菜单。

#### Scenario: 运行 jht
- **WHEN** 用户运行 `jht`
- **THEN** os_tool 菜单启动并可调度 `/www/server/os_tool` 下的脚本
