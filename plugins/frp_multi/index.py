...
ROLE_META = {
    'server': {
        ...
        'tpl_dir': 'server_cfg',
        'binary': 'frps',
        'service_name': 'frps_multi',
        'service_tpl': 'frps.service.tpl',
    },
    'client': {
        ...
        'tpl_dir': 'client_cfg',
        'binary': 'frpc',
        'service_name': 'frpc_multi',
        'service_tpl': 'frpc.service.tpl',
    }
}
...
def getFrpBaseDir():
    """共享 frp 二进制和基础文件目录，优先使用 frp 插件的安装目录"""
    frp_dir = mw.getServerDir() + '/frp'
...
def getRoleBinary(role):
    meta = getRoleMeta(role)
    return getFrpBaseDir() + '/' + meta['binary']
...
def contentReplace(content):
    ...
    content = content.replace('{$SERVER_APP}', getFrpBaseDir())
...
def initDreplace():
    ...
    if os.path.exists(systemDir):
        for role in ROLE_META:
            ...
            systemServiceTpl = getPluginDir() + '/init.d/' + getRoleMeta(role)['service_tpl']
...
def ftOp(method):
    ...
    if mw.isAppleSystem():
        cmd = getServerDir() + '/init.d/' + getPluginName() + ' ' + method + " &"
        data = mw.execShell(cmd)
