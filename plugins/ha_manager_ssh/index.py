...
def _config():
    ...
    if 'monitor_enabled' in saved:
        cfg['monitor_enabled'] = safeBool(cfg.get('monitor_enabled'), True)
    else:
        cfg['monitor_enabled'] = bool(cfg.get('monitor_url'))
    if not cfg.get('monitor_enabled') and not cfg.get('monitor_url'):
        cfg['monitor_enabled'] = False
    if cfg.get('monitor_enabled') and not cfg.get('monitor_url'):
...
def _mysql_apt_result(func, timeout=20):
    cmd = 'python3 {0} {1}'.format(shlex.quote(MYSQL_APT_INDEX), shlex.quote(func))
...
def _health_snapshot(cfg):
    ...
    return {
        ...
        'cloud': {'status': 'normal' if _monitor_enabled(cfg) else 'warning', 'text': '云监控' + ('已启用' if _monitor_enabled(cfg) else ('未启用' if cfg.get('monitor_url') else '未配置'))},
...
def regenerate_host_id():
    ...
    _save_config(cfg)
    state = _state(cfg)
    report_result = _return_data(report_state()) if _monitor_enabled(cfg) else {'status': True, 'msg': '云监控地址为空或未启用，跳过上报'}
...
def save_monitor():
    ...
    _save_config(cfg)
    if not cfg.get('monitor_url'):
        return _return(True, '云监控地址为空，相关功能未启用', cfg)
    if not cfg.get('monitor_enabled'):
        return _return(True, '云监控未启用，不注册、不上报、不拉取任务', cfg)
...
def _post_monitor(cfg, action, payload, signed=True):
    ...
    if not _monitor_enabled(cfg):
        _append_cloud_interaction_log(action, 'skip', msg='云监控未启用', pair_id=payload.get('pair_id') if isinstance(payload, dict) else '', host_id=cfg.get('host_id'))
...
def report_state():
    cfg = _config()
    _append_report_state_log('start', pair_id=cfg.get('pair_id'), host_id=cfg.get('host_id'), monitor_url=cfg.get('monitor_url'), monitor_enabled=cfg.get('monitor_enabled'))
    if not _monitor_enabled(cfg):
        _append_report_state_log('skip', pair_id=cfg.get('pair_id'), host_id=cfg.get('host_id'), msg='云监控地址为空或已禁用')
        return _return(True, '云监控地址为空或未启用，不上传状态', {'hosts': []})
    report_time = _now()
...
def poll_monitor():
    cfg = _config()
    _append_poll_monitor_log('start', pair_id=cfg.get('pair_id'), host_id=cfg.get('host_id'), monitor_url=cfg.get('monitor_url'), monitor_enabled=cfg.get('monitor_enabled'))
    if not _monitor_enabled(cfg):
        ...
        return _return(True, '云监控地址为空或未启用，不轮询期望状态', cfg)
...
def ack_switch_phase(cfg, phase, phase_status, step='', last_error=''):
    if not _monitor_enabled(cfg) or not cfg.get('switch_run_id'):
...
def report_switch_event(cfg, phase, status, text, origin_host_id=None, seq=None, collect_method='local', switch_run_id=None):
    ...
    if not _monitor_enabled(cfg) or not switch_run_id or switch_run_id.startswith('LOCAL_'):
