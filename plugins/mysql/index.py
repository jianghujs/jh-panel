...
def initSlaveStatus(version=''):
    ...
    try:
        ...
        cmd = 'cd /www/server/jh-panel && source venv/bin/activate && python3 plugins/mysql/index.py get_master_rep_slave_user_cmd {"username":"","db":""}'
...
def doFullSync(version=''):
    ...
    dbname = args['db']
    cmd = "cd /www/server/jh-panel && source venv/bin/activate && python3 plugins/mysql/index.py dump_mysql_data {\"db\":'" + dbname + "'}"
    print(cmd)
    stdin, stdout, stderr = ssh.exec_command(cmd)
    result = stdout.read()
    result = result.decode('utf-8')
    ...
    cmd = 'cd /www/server/jh-panel && source venv/bin/activate && python3 plugins/mysql/index.py get_master_rep_slave_user_cmd {"username":"' + db_user + '","db":""}'
    stdin, stdout, stderr = ssh.exec_command(cmd)
    result = stdout.read()
    result = result.decode('utf-8')
    cmd_data = json.loads(result)
