...
''' % (mw_dir, name, getServerDir(), getPluginDir())
    ...
    if mw.isAppleSystem():
        cmd += 'echo "cd $mw_dir && source venv/bin/activate && python3 $script_path/tool_task.py run >> $logs_file 2>&1"' + "\n"
        cmd += 'cd $mw_dir && source venv/bin/activate && python3 $script_path/tool_task.py run >> $logs_file 2>&1' + "\n"
    else:
        cmd += 'echo "cd $mw_dir && source venv/bin/activate && bash $script_path/shell/cpu_usage_file.sh >> $logs_file 2>&1"' + "\n"
        cmd += 'cd $mw_dir && source venv/bin/activate && bash $script_path/shell/cpu_usage.sh >> $logs_file 2>&1' + "\n"
