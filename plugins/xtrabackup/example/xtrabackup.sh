...
if [ $BACKUP_COMPRESS -eq 1 ];then
    xtrabackup --backup --slave-info --compress --compress-threads=4 --user=root  --port=33067 --password=123456 --target-dir=$BACKUP_PATH &>> $LOG_DIR/backup_$timestamp.log
else
    xtrabackup --backup --slave-info --user=root  --port=33067 --password=123456 --target-dir=$BACKUP_PATH &>> $LOG_DIR/backup_$timestamp.log
fi
