...
if [ $choice == "y" ]; then
	...
	done < <(find "$storage_dir" -type d \( -name "node_modules" -o -name "logs" -o -name "run" -o -name ".git" \) -prune -o -type d -print0)
    ...
    find "$storage_dir" -type d \( -name "node_modules" -o -name "logs" -o -name "run" -o -name ".git" \) -prune -o -type d -print | while read dir
    do
		if [[ $dir != *"/app/"* ]]; then
            # 获取相对路径
            relative_path="${dir#$storage_dir}"
