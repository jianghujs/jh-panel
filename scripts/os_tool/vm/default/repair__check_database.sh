...
if ! command -v innochecksum &> /dev/null; then
    ...
    if systemctl is-active mysql-apt &> /dev/null; then
        isMySQLRunning=1
        # 停止MySQL服务器，并通过mysql-apt插件mask服务，避免后台自动拉起
        ...
        if [ "$mysql_stop_result" != "ok" ]; then
            log_echo "停止MySQL服务器失败：$mysql_stop_result"
            exit 1
        fi
    ...
    if [ $isMySQLRunning -eq 1 ]; then
        # 启动MySQL服务器，并通过mysql-apt插件解除mask
        ...
        if [ "$mysql_start_result" != "ok" ]; then
            log_echo "启动MySQL服务器失败：$mysql_start_result"
            exit 1
        fi
