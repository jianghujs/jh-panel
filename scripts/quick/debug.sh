...
for PHP_VER in ${PHP_VER_LIST[@]}; do
	...
	if [ ! -d  /www/server/php/${PHP_VER} ];then
		cd /www/server/jh-panel/plugins/php && bash install.sh install ${PHP_VER}
	fi
	echo "php${PHP_VER} -- end"
done


# cd /www/server/jh-panel/plugins/php-yum && bash install.sh install 74


# mysql
if [ ! -d /www/server/mysql ];then
	# cd /www/server/jh-panel/plugins/mysql && bash install.sh install 5.7


	cd /www/server/jh-panel/plugins/mysql && bash install.sh install 5.6
	# cd /www/server/jh-panel/plugins/mysql && bash install.sh install 8.0
fi
